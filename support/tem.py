...
class data_handle:
    ...
    def __init__(self,GUI):
        ...
        self.GUI.content_shower.itemChanged.connect(self.ques_state_take)
    ...
    def from_local(self):
        # Implement logic to load data from LOCAL_PATH
    ...
    def chapter_load(self):
        ...
        if self.chap_tray[self.GUI.chap_list.currentText()]["chap_content"] is not None:
            ...
            # self.ques_choose = [0]*len(self.chap_content)
    ...
    def chapter_show(self):
        ...
        if self.chap_content:
            ...
            for row_idx, row_data in enumerate(self.chap_content):
                self.GUI.content_shower.setItem(row_idx, 0, QTableWidgetItem(str(row_data['Question'])))
                ...
                if row_idx in self.ques_choose["TN"] or row_idx in  self.ques_choose["TL"]:
                    ...
                    check_item.setCheckState(Qt.CheckState.Unchecked)
                # checkbox=QCheckBox()
                # checkbox.toggled.connect(self.ques_state_take)
                # checkbox.setProperty("row", row_idx)
                self.GUI.content_shower.setItem(row_idx, 5, check_item)
                # if row_idx in self.ques_choose["TN"] or row_idx in  self.ques_choose["TL"]:
                #     self.GUI.content_shower.cellWidget(row_idx, 5).setChecked(True)
                # else:
                #     self.GUI.content_shower.cellWidget(row_idx, 5).setChecked(False)
        ...
        if self.chap_sta:
            ...
            self.GUI.TN_table.setCellWidget(L, 2, item(self.chap_sta["H_N"]))
            self.GUI.TN_table.setCellWidget(L, 3, item(self.chap_sta["M_N"]))
            self.GUI.TN_table.setCellWidget(L, 4, item(self.chap_sta["E_N"]+self.chap_sta["H_N"]+self.chap_sta["M_N"]))

            self.GUI.TL_table.setItem(L, 0, QTableWidgetItem("Total"))
            self.GUI.TL_table.setCellWidget(L, 1, item(self.chap_sta["E_L"]))
            self.GUI.TL_table.setCellWidget(L, 2, item(self.chap_sta["H_L"]))
            self.GUI.TL_table.setCellWidget(L, 3, item(self.chap_sta["M_L"]))
            self.GUI.TL_table.setCellWidget(L, 4, item(self.chap_sta["E_L"]+self.chap_sta["H_L"]+self.chap_sta["M_L"]))
    ...
    def mode(self):
        ...
        if self.GUI.mode.currentText() == "Compose":
        ...
        if self.GUI.mode.currentText() == "Level":
            set_spin_enable(self.GUI.TN_table,TN_r,TN_r,1,4)
            set_spin_enable(self.GUI.TL_table,TL_r,TL_r,1,4)

        if self.GUI.mode.currentText() == "Content":
            set_spin_enable(self.GUI.TN_table,0,TN_r,4,4)
            set_spin_enable(self.GUI.TL_table,0,TL_r,4,4)
    ...
    def make(self):
        ...
        if self.GUI.mode.currentText() == "Compose":
            ...
            print(E_indices, M_indices, H_indices)
            TL_idx = (random.sample(E_indices, E_count) + random.sample(M_indices, M_count)  + random.sample(H_indices, H_count))

        if self.GUI.mode.currentText() == "Level":
            E_count = self.GUI.TN_table.cellWidget(TN_r,1).spin.value()
            M_count = self.GUI.TN_table.cellWidget(TN_r,2).spin.value()
            H_count = self.GUI.TN_table.cellWidget(TN_r,3).spin.value()
            ...
            E_count = self.GUI.TL_table.cellWidget(TL_r,1).spin.value()
            M_count = self.GUI.TL_table.cellWidget(TL_r,2).spin.value()
            H_count = self.GUI.TL_table.cellWidget(TL_r,3).spin.value()
